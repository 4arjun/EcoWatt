...
const Home = () => {
  const tips = [
    "Use energy-efficient appliances, especially for tasks like cooking, heating, and cooling.",
    "Set reminders to turn off lights and appliances when not in use, especially during high-usage periods.",
    "Consider using a programmable thermostat to optimize heating and cooling based on your weekly schedule.",
    "Unplug devices and chargers when not in use, as they can still consume energy even when turned off.",
    "Reduce the use of high-energy appliances like dishwashers and washing machines during peak hours.",
    "Take advantage of natural light during the day to reduce the need for artificial lighting.",
    "Limit the use of space heaters and air conditioners; consider using fans or layering clothing instead.",
    "Schedule laundry and dishwashing during off-peak hours to take advantage of lower energy rates.",
    "Check for drafts in your home and seal them to prevent energy loss, especially during heating or cooling.",
  ];
  ...
  function calculatePredictedBill(usage) {
    ...
    const telescopicSlabs = [
      ...
      { min: 101, max: 150, charge: 5.0 },
      { min: 151, max: 200, charge: 6.8 },
      { min: 201, max: 250, charge: 8.0 },
    ];

    const nonTelescopicSlabs = [
      { min: 0, max: 300, charge: 6.2 },
      { min: 301, max: 350, charge: 7.0 },
      { min: 351, max: 400, charge: 7.35 },
      { min: 401, max: 500, charge: 7.6 },
      { min: 501, max: Infinity, charge: 8.5 },
    ];
  ...
  useEffect(() => {
    const fetchPredictedEnergyConsumption = async () => {
      try {
        const res = await axios.get(`${baseUrl}/api/predictenergyconsumption`);
    ...
    fetchPredictedEnergyConsumption();
    const interval = setInterval(fetchPredictedEnergyConsumption, 3000);
    return () => clearInterval(interval);
  }, []);
  ...
  return (
    <>
          ...
          <div className="stats stats-vertical shadow w-[400px]">
              ...
              <div className="flex text-center justify-around">
                ...
                <div>
                  <div className="stat-value text-5xl">
                    {latestUsage.irms_current?.toFixed(2)}
                  </div>
            ...
            <div className="stat p-6">
              <div className="flex text-center justify-around">
                ...
                <div>
                  <div className="stat-value text-5xl">
                    {latestUsage.irms_power?.toFixed(2)}{" "}
                  </div>
            ...
            <div className="stat p-6">
              <div className="flex text-center justify-around">
                ...
                <div>
                  <div className="stat-value text-5xl">
                    {latestUsage.peak_power?.toFixed(2)}
                  </div>
                  <div className="stat-desc ml-10">Watt</div>
                </div>
              </div>
            ...
            <div className="stat p-6">
              <div className="flex text-center justify-around">
                ...
                <div>
                  <div className="stat-value text-5xl">
                    {latestUsage.usage_value?.toFixed(2)}{" "}
                  </div>
        ...
        <div id="limiter" className="m-5 rounded-lg">
          ...
          <div className="flex-col">
            <div className="stats w-[400px] flex flex-col mb-14">
              <div className="stat stat-desc text-2xl">
                Stop supply after a limit.
              </div>
              <div className="stat flex">
                <form className="flex flex-col h-full justify-center gap-5 w-full">
                  ...
                  <select
                    name="limitter_value"
                    id="limit_value"
                    className="w-full text-2xl rounded-lg"
                  >
      ...
      <div id="tips-container" className="flex gap-2 h-[20%] items-center">
        ...
        <p id="theParaEl" className="text-2xl">
          {currentTip}
        </p>
      </div>
    </>
